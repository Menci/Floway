...
### Responses no longer sends `ping` keep-alive frames, and long WebSocket turns need a client heartbeat

Two `{ "type": "ping" }` sources are gone from the Responses streaming surface. The WebSocket transport no longer synthesizes one every 15 s while a turn is idle, and the Messages-backed translation no longer converts an upstream Anthropic `ping` into one. `ping` is not a member of the OpenResponses 2026-04-24 streaming-event union, so a client that validated the stream against the union previously failed any turn long enough to produce one; clients that treated `ping` as a liveness signal must stop expecting it. The Responses SSE transport is unchanged — its keep-alive was already an SSE comment line, invisible to the event stream.
