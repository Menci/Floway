  ...
  await withMockedFetch(
    ...
    async () => {
      await copilotAuthedFetch(
        ...
        extraHeaders ? { headers: extraHeaders, fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p } : { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p },
      );
    },
  );
