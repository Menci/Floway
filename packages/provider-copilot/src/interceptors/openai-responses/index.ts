// Copilot-only OpenAI Responses workarounds. The chain is a boundary the Copilot
// provider runs inside its own `callX` methods, so the gateway main flow
// never knows that Copilot has OpenAI Responses interceptors at all.
...
] as const satisfies readonly CopilotOpenAIResponsesBoundaryInterceptor[];
