...
const response = (output: OpenAIResponsesOutputItem[]): OpenAIResponsesResult => ({
  id: 'resp_test',
  object: 'response',
  model: 'test-model',
  output,
...
const eventAt = <TType extends OpenAIResponsesStreamEvent['type']>(
  frames: ProtocolFrame<OpenAIResponsesStreamEvent>[],
  type: TType,
): Extract<OpenAIResponsesStreamEvent, { type: TType }> => {
  ...
  return frame.event as Extract<OpenAIResponsesStreamEvent, { type: TType }>;
};

test('normalizes queued output and reuses its public id when the item is added', async () => {
  const item: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_queued', summary: [], encrypted_content: 'queued state' };
...
test('normalizes each reasoning lifecycle observation with its own upstream id and state', async () => {
  const added: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_added', summary: [], encrypted_content: 'opaque added' };
  const done: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_done', summary: [], encrypted_content: 'opaque done' };
  const terminal: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_terminal', summary: [], encrypted_content: 'opaque terminal' };
  let release!: () => void;
  const gate = new Promise<void>(resolve => { release = resolve; });
  const upstream = (async function* (): AsyncGenerator<ProtocolFrame<OpenAIResponsesStreamEvent>> {
  ...
  release();
  const rest: ProtocolFrame<OpenAIResponsesStreamEvent>[] = [];
...
test.each([
  ...
  const { result } = await runStream(openaiResponsesResultToEvents(response([fixture as OpenAIResponsesOutputItem])));
  const frames = await collect(result);
...
test.each(uncarriedOutputItems)('randomizes a Copilot %s item without exposing its raw id', async (prefix, fixture) => {
  const item = fixture as OpenAIResponsesOutputItem;
  const { result } = await runStream([
    eventFrame(outputItemEvent('added', 0, item)),
    eventFrame(outputItemEvent('done', 0, item)),
...
test('preserves shell command events that carry no item id', async () => {
  const addedItem: OpenAIResponsesOutputItem = {
    type: 'shell_call',
    id: 'sh_raw',
    call_id: 'call_shell',
    action: { commands: [] },
  ...
  const doneItem: OpenAIResponsesOutputItem = {
    ...addedItem,
    action: { commands: ['ls -a ~/Desktop'] },
    status: 'completed',
  };
  const commandEvents: OpenAIResponsesStreamEvent[] = [
...
test('rewrites reasoning-text item ids', async () => {
  const item: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_raw', summary: [] };
  const { result } = await runStream([
    eventFrame(outputItemEvent('added', 0, item)),
...
test('fails closed on unknown output types before yielding a raw id', async () => {
  const unknown = { type: 'future_call', id: 'raw_future' } as unknown as OpenAIResponsesOutputItem;
  const { result } = await runStream([
    eventFrame(outputItemEvent('added', 0, unknown)),
    doneFrame(),
  ]);

  await expect(collect(result)).rejects.toThrow("Unsupported Copilot OpenAI Responses output item type 'future_call'");
});

test('rejects a repeated output_item.added observation', async () => {
  const item: OpenAIResponsesOutputItem = { type: 'message', id: 'msg_raw', status: 'in_progress', role: 'assistant', content: [] };
  const { result } = await runStream([
    eventFrame(outputItemEvent('added', 0, item)),
    eventFrame(outputItemEvent('added', 0, item)),
  ]);
...
test('rejects replay state without the upstream id that authenticates it', async () => {
  const item = { type: 'reasoning', summary: [], encrypted_content: 'opaque state' } as unknown as OpenAIResponsesOutputItem;
...
test.each(['toString', 'constructor', '__proto__'])('does not accept Object prototype key %s as an output item type', async type => {
  const unknown = { type, id: 'raw_future' } as unknown as OpenAIResponsesOutputItem;
  const { result } = await runStream([eventFrame(outputItemEvent('added', 0, unknown))]);

  await expect(collect(result)).rejects.toThrow(`Unsupported Copilot OpenAI Responses output item type '${type}'`);
});
...
test('fails closed on unknown event envelopes that could hide an item id', async () => {
  ...
  await expect(collect(result)).rejects.toThrow("Unsupported Copilot OpenAI Responses stream event type 'response.future'");
});
...
test('forwards repeated done frames with stable public identity and each frame own content', async () => {
  const first: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_first', summary: [], encrypted_content: 'first' };
  const second: OpenAIResponsesOutputItem = { type: 'reasoning', id: 'rs_second', summary: [], encrypted_content: 'second' };
  const { result } = await runStream([
...
test('normalizes a failed response with an open item instead of suppressing the failure', async () => {
  const partial: OpenAIResponsesOutputItem = { type: 'message', id: 'msg_failed', status: 'in_progress', role: 'assistant', content: [] };
  const { result } = await runStream([
