...
describe('refreshClaudeCodeAccessToken', () => {
  ...
  test('caller abort cancels a stalled token response body', async () => {
    ...
    const result = refreshClaudeCodeAccessToken('rt_old', directFetcher, controller.signal);
  ...
  test('rejects and cancels an oversized token response body', async () => {
    ...
    const error = await refreshClaudeCodeAccessToken('rt_old', directFetcher).catch(cause => cause as unknown);
