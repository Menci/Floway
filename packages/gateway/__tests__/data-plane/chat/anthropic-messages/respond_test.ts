...
const callRespond = async (wantsStream: boolean): Promise<Response> => {
  ...
  app.get('/', async c => {
    ...
    const response = await respondAnthropicMessages(c, result, wantsStream, makeRespondCtx());
    captured = response;
    return response;
  });
  await app.request('/');
  if (!captured) throw new Error('respondAnthropicMessages did not produce a Response');
  return captured;
};

test('respondAnthropicMessages forwards upstream headers and strips hop-by-hop / framing / cookie headers on the non-streaming JSON response', async () => {
...
test('respondAnthropicMessages forwards upstream headers and strips hop-by-hop / framing / cookie headers on the streaming SSE response', async () => {
