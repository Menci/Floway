...
const { responsesServe } = await import('../../../../src/data-plane/chat/responses/serve.ts');
const { expandPreviousResponseId } = await import('../../../../src/data-plane/chat/responses/serve-prep.ts');
...
test('generate filters translated targets after affinity leaves encrypted agent content', async () => {
  installRepo();
    ...
    payload: makePayload({
      input: [{
        ...
        content: [
          ...
          { type: 'encrypted_content', encrypted_content: 'foreign-opaque' },
        ],
      }],
    }),
    ctx: makeGatewayCtx(),
...
test('generate preserves encrypted agent content for a native Responses candidate', async () => {
  installRepo();
  const callMessages = vi.fn();
  ...
  const input = [{
    ...
    recipient: '/root/reviewer',
    content: [
      { type: 'input_text' as const, text: 'Message Type: NEW_TASK\nPayload:\n' },
      { type: 'encrypted_content' as const, encrypted_content: 'foreign-opaque' },
    ],
  }];
  ...
  assertEquals(callMessages.mock.calls.length, 0);
  assertEquals(callResponses.mock.calls.length, 1);
  assertEquals(capturedBodies[0]?.input, input);
});
