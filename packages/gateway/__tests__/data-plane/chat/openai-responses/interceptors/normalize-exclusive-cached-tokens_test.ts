...
import { withExclusiveCachedTokensNormalized } from '../../../../../src/data-plane/chat/openai-responses/interceptors/normalize-exclusive-cached-tokens.ts';
...
test('leaves a normalized response billable instead of underflowing', async () => {
  expect(() => billableUsageFromOpenAIResponsesResult({ usage: exclusiveUsage() })).toThrowError(RangeError);

  const response = await run(invocation(new Set()), exclusiveUsage());
  assertEquals(
    billableUsageFromOpenAIResponsesResult(response),
