...
test('/api/performance/overview ORs repeated values within a filter and ANDs dimensions', async () => {
  ...
  const response = await requestApp('/api/performance/overview?start=2026-04-30T00&end=2026-05-01T00&filter_model=gpt-5&filter_model=claude-opus-4-7&filter_upstream=copilot%3A1', { headers: { 'x-api-key': apiKey.key } });
