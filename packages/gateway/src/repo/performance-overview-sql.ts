...
const seriesGroupSql = (source: string) => `CASE settings.series_group_by
  ...
  WHEN 'model' THEN ${source}.model_json
...
const performanceBreakdownSql: readonly PerformanceBreakdownSql[] = [
  ...
  { axis: 'model', group: source => `${source}.model_json` },
...
const performanceCubeDimensions = [
  'key_id', 'user_id', 'owned', 'model_json', 'upstream', 'operation', 'runtime_location',
] as const;
...
orphan_buckets AS MATERIALIZED (
  ...
  LEFT JOIN performance_summary ON
    ...
    AND performance_summary.model_json = performance_buckets.model_json
...
filtered_histogram AS MATERIALIZED (
  ...
  JOIN performance_buckets ON
    ...
    AND performance_buckets.model_json = filtered_summary.model_json
...
facet_rows AS (
  ...
  GROUP BY user_id
  UNION ALL
  SELECT 'facet', NULL, NULL, NULL, 'model', model_json,
    NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL
  FROM scoped_summary GROUP BY model_json
  UNION ALL
...
const finishOverview = (
  rows: readonly PerformanceOverviewSqlRow[],
  groupBy: PerformanceOverviewQueryOptions['groupBy'],
): PerformanceOverviewResult => {
  ...
  for (const row of rows) {
    ...
    if (row.row_kind === 'facet') {
      ...
      const value = row.dimension === 'model'
        ? decodeOpaqueSqlText(row.facet_value, 'performance overview model facet')
        : row.facet_value;
    ...
    const axis = row.axis as PerformanceOverviewAxis;
    const group = axis === 'model' || (axis === 'series' && groupBy === 'model')
      ? decodeOpaqueSqlText(row.group_value, 'performance overview model group')
      : row.group_value;
    records.get(axis)!.push({
      bucket: row.bucket,
      group,
