import { assertSameStoredOpenAIResponsesItem, scopedOpenAIResponsesKey } from './openai-responses-clone.ts';
import { hashOpenAIResponsesJson } from './openai-responses-hash.ts';
import {
  prepareStoredOpenAIResponsesPayload,
  writePreparedStoredOpenAIResponsesPayload,
  parseStoredOpenAIResponsesPayload,
  type PreparedStoredOpenAIResponsesPayload,
} from './openai-responses-payload.ts';
import { quantizeOpenAIResponsesRefreshedAt, OPENAI_RESPONSES_REFRESH_GRANULARITY_MS } from './openai-responses-retention.ts';
import { decodeOpenAIResponsesSnapshotItemIds, encodeOpenAIResponsesSnapshotItemIds } from './openai-responses-snapshot-codec.ts';
import { SPILLED_FILE_STAGE_GRACE_MS } from './spilled-files-policy.ts';
import { runStatements } from './sql-batch.ts';
import type {
  OpenAIResponsesItemsRepo,
  OpenAIResponsesSnapshotsRepo,
  StoredOpenAIResponsesItem,
  StoredOpenAIResponsesSnapshot,
} from './types.ts';
import type { SqlDatabase, SqlPreparedStatement, SqlResult } from '@floway-dev/platform';

// The schema keeps the pre-rename spelling — `responses_items`,
// `responses_snapshots`, `api_keys.responses_retention_seconds`, and the
// indices and triggers over them. `responses_items` is the largest table this
// deployment carries and a rename of it is not viable there, so the SQL below
// deliberately reads names the surrounding code no longer uses.

const OPENAI_RESPONSES_ITEM_COLUMNS = 'id, api_key_id, payload_json, item_hash, payload_hash, payload_file_key, refreshed_at';
const OPENAI_RESPONSES_IN_QUERY_CHUNK_SIZE = 80;
const OPENAI_RESPONSES_INSERT_CHUNK_SIZE = 14;
const OPENAI_RESPONSES_REFRESH_CHUNK_SIZE = 45;

const mapSequentially = async <T, U>(values: readonly T[], mapper: (value: T) => Promise<U>): Promise<U[]> => {
  const mapped: U[] = [];
  for (const value of values) mapped.push(await mapper(value));
  return mapped;
};

const uniqueOpenAIResponsesItems = (items: readonly StoredOpenAIResponsesItem[]): StoredOpenAIResponsesItem[] => {
  const unique = new Map<string, StoredOpenAIResponsesItem>();
  for (const source of items) {
    const item = { ...source, refreshedAt: quantizeOpenAIResponsesRefreshedAt(source.refreshedAt) };
    const key = scopedOpenAIResponsesKey(item.apiKeyId, item.id);
    const existing = unique.get(key);
    if (existing === undefined) unique.set(key, item);
    else assertSameStoredOpenAIResponsesItem(item, existing);
  }
  return [...unique.values()];
};

interface OpenAIResponsesItemRow {
  id: string;
  api_key_id: string;
  payload_json: string;
  item_hash: string;
  payload_hash: string;
  payload_file_key: string | null;
  refreshed_at: number;
}

const toStoredOpenAIResponsesItem = async (row: OpenAIResponsesItemRow): Promise<StoredOpenAIResponsesItem> => ({
  id: row.id,
  apiKeyId: row.api_key_id,
  payload: await parseStoredOpenAIResponsesPayload(row.id, row.payload_json, row.payload_file_key),
  itemHash: row.item_hash,
  refreshedAt: row.refreshed_at,
});

interface PreparedOpenAIResponsesItem {
  item: StoredOpenAIResponsesItem;
  payload: PreparedStoredOpenAIResponsesPayload;
  payloadHash: string;
}

export class SqlOpenAIResponsesItemsRepo implements OpenAIResponsesItemsRepo {
  constructor(private readonly db: SqlDatabase) {}

  async lookupMany(apiKeyId: string, ids: readonly string[], earliestVisibleCutoff: number): Promise<StoredOpenAIResponsesItem[]> {
    const rows = await this.lookupByColumn(apiKeyId, 'id', ids, earliestVisibleCutoff);
    const order = new Map([...new Set(ids)].map((id, index) => [id, index]));
    return rows.toSorted((a, b) => order.get(a.id)! - order.get(b.id)!);
  }

  async lookupManyByItemHash(apiKeyId: string, hashes: readonly string[], earliestVisibleCutoff: number): Promise<StoredOpenAIResponsesItem[]> {
    return await this.lookupByColumn(apiKeyId, 'item_hash', hashes, earliestVisibleCutoff);
  }

  private async lookupByColumn(
    apiKeyId: string,
    column: 'id' | 'item_hash',
    values: readonly string[],
    earliestVisibleCutoff: number,
  ): Promise<StoredOpenAIResponsesItem[]> {
    const unique = [...new Set(values)];
    if (unique.length === 0) return [];
    const queries: Promise<SqlResult<OpenAIResponsesItemRow>>[] = [];
    for (let index = 0; index < unique.length; index += OPENAI_RESPONSES_IN_QUERY_CHUNK_SIZE) {
      const chunk = unique.slice(index, index + OPENAI_RESPONSES_IN_QUERY_CHUNK_SIZE);
      const placeholders = chunk.map(() => '?').join(', ');
      const orderSql = column === 'item_hash' ? ' ORDER BY refreshed_at DESC, id ASC' : '';
      queries.push(this.db
        .prepare(
          `SELECT ${OPENAI_RESPONSES_ITEM_COLUMNS} FROM responses_items
           WHERE api_key_id = ? AND refreshed_at >= ? AND ${column} IN (${placeholders})${orderSql}`,
        )
        .bind(apiKeyId, earliestVisibleCutoff, ...chunk)
        .all<OpenAIResponsesItemRow>());
    }
    const rows = (await Promise.all(queries)).flatMap(result => result.results);
    const hydrated = await mapSequentially(rows, async row => await this.hydrateCurrentItem(row, earliestVisibleCutoff));
    const wanted = new Set(unique);
    return hydrated.flatMap(item =>
      item !== null && (column === 'id' ? wanted.has(item.id) : wanted.has(item.itemHash))
        ? [item]
        : []);
  }

  private async hydrateCurrentItem(initialRow: OpenAIResponsesItemRow, earliestVisibleCutoff: number): Promise<StoredOpenAIResponsesItem | null> {
    let row = initialRow;
    for (;;) {
      try {
        return await toStoredOpenAIResponsesItem(row);
      } catch (error) {
        const current = await this.db
          .prepare(
            `SELECT ${OPENAI_RESPONSES_ITEM_COLUMNS} FROM responses_items
             WHERE id = ? AND api_key_id = ? AND refreshed_at >= ?`,
          )
          .bind(row.id, row.api_key_id, earliestVisibleCutoff)
          .first<OpenAIResponsesItemRow>();
        if (current === null) return null;
        if (current.payload_json === row.payload_json && current.payload_file_key === row.payload_file_key) throw error;
        row = current;
      }
    }
  }

  async insertMany(items: readonly StoredOpenAIResponsesItem[], earliestVisibleCutoff: number): Promise<void> {
    const unique = uniqueOpenAIResponsesItems(items);
    const existing = await this.lookupExistingItems(unique, earliestVisibleCutoff);
    for (const item of unique) {
      const actual = existing.get(scopedOpenAIResponsesKey(item.apiKeyId, item.id));
      if (actual !== undefined) assertSameStoredOpenAIResponsesItem(item, actual);
    }

    const pending = unique.filter(item => !existing.has(scopedOpenAIResponsesKey(item.apiKeyId, item.id)));
    const prepared = await mapSequentially(pending, async item => ({
      item,
      payload: await prepareStoredOpenAIResponsesPayload(item.id, item.apiKeyId, item.payload),
      payloadHash: await hashOpenAIResponsesJson(item.payload),
    }));
    await this.stageFiles(prepared);
    for (const entry of prepared) await writePreparedStoredOpenAIResponsesPayload(entry.payload);

    const statements: SqlPreparedStatement[] = [];
    for (let index = 0; index < prepared.length; index += OPENAI_RESPONSES_INSERT_CHUNK_SIZE) {
      const chunk = prepared.slice(index, index + OPENAI_RESPONSES_INSERT_CHUNK_SIZE);
      const values = chunk.map(() => '(?, ?, ?, ?, ?, ?, ?)').join(', ');
      statements.push(this.db
        .prepare(
          `WITH incoming (${OPENAI_RESPONSES_ITEM_COLUMNS}) AS (VALUES ${values})
           INSERT INTO responses_items (${OPENAI_RESPONSES_ITEM_COLUMNS})
           SELECT incoming.* FROM incoming
           JOIN api_keys ON api_keys.id = incoming.api_key_id
           WHERE true
           ON CONFLICT (id, api_key_id) DO UPDATE SET
             payload_json = excluded.payload_json,
             item_hash = excluded.item_hash,
             payload_hash = excluded.payload_hash,
             payload_file_key = excluded.payload_file_key,
             refreshed_at = excluded.refreshed_at
           WHERE responses_items.refreshed_at < ?`,
        )
        .bind(
          ...chunk.flatMap(({ item, payload, payloadHash }) => [
            item.id,
            item.apiKeyId,
            payload.payloadJson,
            item.itemHash,
            payloadHash,
            payload.file?.key ?? null,
            item.refreshedAt,
          ]),
          earliestVisibleCutoff,
        ));
    }
    await runStatements(this.db, statements);

    const persisted = await this.lookupExistingItems(unique, earliestVisibleCutoff);
    for (const item of unique) {
      const actual = persisted.get(scopedOpenAIResponsesKey(item.apiKeyId, item.id));
      if (actual === undefined) throw new Error(`OpenAI Responses item disappeared after insert: ${item.id}`);
      assertSameStoredOpenAIResponsesItem(item, actual);
    }
    const refreshGroups = Map.groupBy(unique.filter(item => {
      const actual = persisted.get(scopedOpenAIResponsesKey(item.apiKeyId, item.id))!;
      return actual.refreshedAt < item.refreshedAt;
    }), item => item.refreshedAt);
    for (const [refreshedAt, group] of refreshGroups) {
      await this.refreshMany(group, refreshedAt, earliestVisibleCutoff);
    }
  }

  private async stageFiles(prepared: readonly PreparedOpenAIResponsesItem[]): Promise<void> {
    const files = prepared.flatMap(({ item, payload }) => payload.file === null
      ? []
      : [{ fileKey: payload.file.key, apiKeyId: item.apiKeyId, itemId: item.id }]);
    if (files.length === 0) return;
    await this.db
      .prepare(
        `INSERT INTO spilled_files (file_key, owner_kind, owner_key, state, collect_after)
         SELECT
           json_extract(value, '$.fileKey'),
           'responses-item',
           json_array(json_extract(value, '$.apiKeyId'), json_extract(value, '$.itemId')),
           'staged',
           ?
         FROM json_each(?)`,
      )
      .bind(Date.now() + SPILLED_FILE_STAGE_GRACE_MS, JSON.stringify(files))
      .run();
  }

  private async lookupExistingItems(
    items: readonly Pick<StoredOpenAIResponsesItem, 'id' | 'apiKeyId'>[],
    earliestVisibleCutoff: number,
  ): Promise<Map<string, StoredOpenAIResponsesItem>> {
    const idsByApiKey = Map.groupBy(items, item => item.apiKeyId);
    const rows = (await Promise.all([...idsByApiKey].map(async ([apiKeyId, scoped]) =>
      await this.lookupMany(apiKeyId, scoped.map(item => item.id), earliestVisibleCutoff)))).flat();
    return new Map(rows.map(item => [scopedOpenAIResponsesKey(item.apiKeyId, item.id), item]));
  }

  async refreshMany(
    items: readonly StoredOpenAIResponsesItem[],
    refreshedAt: number,
    earliestVisibleCutoff: number,
  ): Promise<void> {
    const quantizedRefreshedAt = quantizeOpenAIResponsesRefreshedAt(refreshedAt);
    const idsByApiKey = Map.groupBy(
      [...new Map(items.map(item => [scopedOpenAIResponsesKey(item.apiKeyId, item.id), item])).values()],
      item => item.apiKeyId,
    );
    const statements: SqlPreparedStatement[] = [];
    for (const [apiKeyId, scoped] of idsByApiKey) {
      const expected = await mapSequentially(scoped, async item => ({
        item,
        payloadHash: await hashOpenAIResponsesJson(item.payload),
      }));
      for (let index = 0; index < expected.length; index += OPENAI_RESPONSES_REFRESH_CHUNK_SIZE) {
        const chunk = expected.slice(index, index + OPENAI_RESPONSES_REFRESH_CHUNK_SIZE);
        const values = chunk.map(() => '(?, ?)').join(', ');
        statements.push(this.db
          .prepare(
            `WITH expected (id, payload_hash) AS (VALUES ${values})
             UPDATE responses_items SET refreshed_at = ?
             WHERE api_key_id = ?
               AND refreshed_at >= ?
               AND refreshed_at < ?
               AND EXISTS (
                 SELECT 1 FROM expected
                 WHERE expected.id = responses_items.id
                   AND expected.payload_hash = responses_items.payload_hash
               )`,
          )
          .bind(
            ...chunk.flatMap(({ item, payloadHash }) => [item.id, payloadHash]),
            quantizedRefreshedAt,
            apiKeyId,
            earliestVisibleCutoff,
            quantizedRefreshedAt,
          ));
      }
    }
    await runStatements(this.db, statements);
    const persisted = await this.lookupExistingItems(items, earliestVisibleCutoff);
    const missing = items.find(item => !persisted.has(scopedOpenAIResponsesKey(item.apiKeyId, item.id)));
    if (missing !== undefined) throw new Error(`OpenAI Responses item disappeared before retention refresh: ${missing.id}`);
    for (const item of items) {
      assertSameStoredOpenAIResponsesItem(item, persisted.get(scopedOpenAIResponsesKey(item.apiKeyId, item.id))!);
    }
  }

  async deleteExpiredBatch(apiKeyId: string, now: number, limit: number): Promise<number> {
    // D1 derives meta.changes from total_changes(), so payload retirement
    // triggers can inflate it. RETURNING counts only OpenAI Responses item rows.
    // https://github.com/cloudflare/workerd/blob/0c0f9656d3f78c75a7dc011e0c17dd85e438b44c/src/cloudflare/internal/test/d1/d1-mock.js#L83-L131
    // https://www.sqlite.org/c3ref/total_changes.html
    // https://www.sqlite.org/lang_returning.html
    const active = await this.db
      .prepare(
        `DELETE FROM responses_items WHERE rowid IN (
           SELECT stored.rowid
           FROM api_keys
           CROSS JOIN responses_items AS stored
           WHERE api_keys.id = ?
             AND api_keys.deleted_at IS NULL
             AND api_keys.responses_retention_seconds > 0
             AND stored.api_key_id = api_keys.id
             AND stored.refreshed_at < ? - api_keys.responses_retention_seconds * 1000
           ORDER BY stored.refreshed_at, stored.rowid
           LIMIT ?
         )
         RETURNING rowid`,
      )
      .bind(apiKeyId, now - OPENAI_RESPONSES_REFRESH_GRANULARITY_MS, limit)
      .all<{ rowid: number }>();
    const activeDeleted = active.results.length;
    if (activeDeleted >= limit) return activeDeleted;
    const inactive = await this.db
      .prepare(
        `DELETE FROM responses_items WHERE rowid IN (
           SELECT stored.rowid FROM responses_items AS stored
           WHERE stored.api_key_id = ?
             AND NOT EXISTS (
               SELECT 1 FROM api_keys
               WHERE api_keys.id = stored.api_key_id
                 AND api_keys.deleted_at IS NULL
                 AND api_keys.responses_retention_seconds > 0
             )
           ORDER BY stored.refreshed_at, stored.rowid
           LIMIT ?
         )
         RETURNING rowid`,
      )
      .bind(apiKeyId, limit - activeDeleted)
      .all<{ rowid: number }>();
    return activeDeleted + inactive.results.length;
  }

  async findOldestRefreshedAt(apiKeyId: string): Promise<number | null> {
    const row = await this.db
      .prepare('SELECT refreshed_at FROM responses_items WHERE api_key_id = ? ORDER BY refreshed_at LIMIT 1')
      .bind(apiKeyId)
      .first<{ refreshed_at: number }>();
    return row?.refreshed_at ?? null;
  }

  async deleteAll(): Promise<void> {
    await this.db.prepare('DELETE FROM responses_items').run();
  }
}

interface OpenAIResponsesSnapshotRow {
  id: string;
  api_key_id: string;
  item_ids_json: string;
  refreshed_at: number;
}

const toStoredOpenAIResponsesSnapshot = (row: OpenAIResponsesSnapshotRow): StoredOpenAIResponsesSnapshot => {
  return {
    id: row.id,
    apiKeyId: row.api_key_id,
    itemIds: decodeOpenAIResponsesSnapshotItemIds(row.item_ids_json, row.id, row.api_key_id),
    refreshedAt: row.refreshed_at,
  };
};

export class SqlOpenAIResponsesSnapshotsRepo implements OpenAIResponsesSnapshotsRepo {
  constructor(private readonly db: SqlDatabase) {}

  async lookup(apiKeyId: string, id: string, earliestVisibleCutoff: number): Promise<StoredOpenAIResponsesSnapshot | null> {
    const row = await this.db
      .prepare(
        `SELECT id, api_key_id, item_ids_json, refreshed_at FROM responses_snapshots
         WHERE id = ? AND api_key_id = ? AND refreshed_at >= ?`,
      )
      .bind(id, apiKeyId, earliestVisibleCutoff)
      .first<OpenAIResponsesSnapshotRow>();
    return row === null ? null : toStoredOpenAIResponsesSnapshot(row);
  }

  async insert(snapshot: StoredOpenAIResponsesSnapshot): Promise<void> {
    const quantized = {
      ...snapshot,
      refreshedAt: quantizeOpenAIResponsesRefreshedAt(snapshot.refreshedAt),
    };
    await this.db
      .prepare(
        `INSERT INTO responses_snapshots (id, api_key_id, item_ids_json, refreshed_at)
         VALUES (?, ?, ?, ?)
         ON CONFLICT (id, api_key_id) DO UPDATE SET
           item_ids_json = excluded.item_ids_json,
           refreshed_at = excluded.refreshed_at
         WHERE responses_snapshots.refreshed_at < excluded.refreshed_at`,
      )
      .bind(
        quantized.id,
        quantized.apiKeyId,
        encodeOpenAIResponsesSnapshotItemIds(quantized.itemIds),
        quantized.refreshedAt,
      )
      .run();
  }

  async deleteExpiredBatch(apiKeyId: string, now: number, limit: number): Promise<number> {
    const active = await this.db
      .prepare(
        `DELETE FROM responses_snapshots WHERE rowid IN (
           SELECT stored.rowid
           FROM api_keys
           CROSS JOIN responses_snapshots AS stored
           WHERE api_keys.id = ?
             AND api_keys.deleted_at IS NULL
             AND api_keys.responses_retention_seconds > 0
             AND stored.api_key_id = api_keys.id
             AND stored.refreshed_at < ? - api_keys.responses_retention_seconds * 1000
           ORDER BY stored.refreshed_at, stored.rowid
           LIMIT ?
         )
         RETURNING rowid`,
      )
      .bind(apiKeyId, now - OPENAI_RESPONSES_REFRESH_GRANULARITY_MS, limit)
      .all<{ rowid: number }>();
    const activeDeleted = active.results.length;
    if (activeDeleted >= limit) return activeDeleted;
    const inactive = await this.db
      .prepare(
        `DELETE FROM responses_snapshots WHERE rowid IN (
           SELECT stored.rowid FROM responses_snapshots AS stored
           WHERE stored.api_key_id = ?
             AND NOT EXISTS (
               SELECT 1 FROM api_keys
               WHERE api_keys.id = stored.api_key_id
                 AND api_keys.deleted_at IS NULL
                 AND api_keys.responses_retention_seconds > 0
             )
           ORDER BY stored.refreshed_at, stored.rowid
           LIMIT ?
         )
         RETURNING rowid`,
      )
      .bind(apiKeyId, limit - activeDeleted)
      .all<{ rowid: number }>();
    return activeDeleted + inactive.results.length;
  }

  async findOldestRefreshedAt(apiKeyId: string): Promise<number | null> {
    const row = await this.db
      .prepare('SELECT refreshed_at FROM responses_snapshots WHERE api_key_id = ? ORDER BY refreshed_at LIMIT 1')
      .bind(apiKeyId)
      .first<{ refreshed_at: number }>();
    return row?.refreshed_at ?? null;
  }

  async deleteAll(): Promise<void> {
    await this.db.prepare('DELETE FROM responses_snapshots').run();
  }
}
