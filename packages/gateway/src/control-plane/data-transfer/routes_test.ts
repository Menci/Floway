...
test('import rejects a usage record whose inputAboveTokens is a negative or fractional value', async () => {
  ...
  const fractional = await doImport(app, 'replace', latestImportData({ usage: [{ ...USAGE_2, inputAboveTokens: 272000.5 }] }));
  assertEquals(fractional.status, 400);
  assertEquals(fractional.body.error, 'invalid usage at index 0: inputAboveTokens, when present, must be a non-negative safe integer or null');
});
