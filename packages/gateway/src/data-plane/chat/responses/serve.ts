...
export const responsesServe = {
  generate: async (args: ResponsesServeGenerateArgs): Promise<ExecuteResult<ProtocolFrame<ResponsesStreamEvent>>> => {
    ...
    // When the request rode in on an alias, rewrite `prepared.model` to the
    // chosen target's public id so the downstream provider dispatches under
    // the real model name.
    return await iterateCandidates(
      plan.candidates,
      'responsesServe.generate',
      candidate => {
        if (plan.aliased) plan.prepared.model = candidate.model.id;
