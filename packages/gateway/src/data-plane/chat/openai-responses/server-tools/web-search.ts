...
// `OpenAIResponsesHostedToolType` so the type union and runtime check can't drift.
...
export const isHostedWebSearchTool = (tool: OpenAIResponsesTool): tool is OpenAIResponsesHostedTool =>
...
export const canonicalizeWebSearchTool = (raw: OpenAIResponsesTool): OpenAIResponsesHostedTool | undefined => {
  if (!isHostedWebSearchTool(raw)) return undefined;
  const canonical: OpenAIResponsesHostedTool = {
    ...raw,
    type: raw.type,
...
const ALPHA_SEARCH_SETTING_KEYS = [
  ...
  'allowed_callers',
  'external_web_access',
] as const satisfies readonly (keyof OpenAIResponsesHostedTool)[];

export const alphaSearchSettingsFromHosted = (
  hosted: OpenAIResponsesHostedTool | undefined,
...
const extractFilters = (tool: OpenAIResponsesHostedTool): WebSearchFilters => {
...
export const synthesizeWebSearchCallId = (): string => createRandomOpenAIResponsesItemId('web_search_call');
            ...
            dispatcher: ({ intercepted, loopState }) => {
              ...
              return [{
                ...
                run: async function* run() {
                  ...
                  const item: ServerToolOutputItem & Omit<OpenAIResponsesOutputWebSearchCall, 'id'> = state.includeSearchResults
