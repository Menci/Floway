...
const missingUsage = (): never => {
  throw new TypeError('OpenAI Responses compaction upstream reported no token usage; the compaction resource requires it');
};
...
// upstream's own compacted list. `object` is restated because `OpenAIResponsesResult`
