// POST /v1/completions and /completions — OpenAI Completions passthrough.
// The endpoint sits outside the chat source/target executor:
...
export const openaiCompletions = async (c: Context): Promise<Response> => {
  ...
  const request = prepareJsonModelRequest(requestBody.bytes, 'OpenAI Completions');
  ...
  const settleUsage = (): TokenUsage | null =>
    streamingUsageBlock === null ? null : tokenUsageFromOpenAICompletionsUsage(streamingUsageBlock, streamingServiceTier, declaredExclusive(), servingIdentity());
    ...
    response: wantsStream
      ...
      : {
          ...
          extractBilling: (body: unknown) => {
            ...
            return tokenUsageFromOpenAICompletionsUsage(usage, tier, declaredExclusive(), servingIdentity());
          },
        },
  });
