...
const runFetch = (
  ...
  const flight = await getRepo().upstreams.claimModelsCacheFlight(key, generation);
  ...
  try {
    ...
    const persisted = flight === null
      ? false
      : await getRepo().upstreams.saveModelsCache(key, generation, entry, flight);
    ...
    // persisted.
    if (flight !== null) {
      await getRepo().upstreams.saveModelsCacheError(key, generation, { message: errorMessage(err), at: Date.now() }, flight);
    }
    throw err;
  }
});
