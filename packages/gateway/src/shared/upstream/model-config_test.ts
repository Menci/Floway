    ...
    [
      {
        ...
        pricing: { units: { input: 'tokens_1m', output: 'tokens_1m', input_cache_read: 'tokens_1m', input_cache_write: 'tokens_1m' }, entries: [{ rates: { input: 2.5, output: 15, input_cache_read: 0.25, input_cache_write: 3.75 } }] },
...
test('modelsField rejects pricing with a negative input', () => {
  assertThrows(
    () =>
      modelsField(
        [{ upstreamModelId: 'gpt-prod', endpoints: { chatCompletions: {} }, pricing: { units: { input: 'tokens_1m', output: 'tokens_1m' }, entries: [{ rates: { input: -1, output: 1 } }] } }],
        'azure',
      ),
    Error,
