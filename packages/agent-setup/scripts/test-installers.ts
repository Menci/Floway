...
test('zed', 'projects the catalog into available_models and keeps unrelated settings', async t => {
  ...
  t.equal(provider.available_models.map(entry => entry.name).join(','), 'claude-opus-4-6[1m],gpt-5.6,plain-chat,effort-only,floor-only,ceiling-only,zero-limits', 'chat models only, in catalog order');
});
...
test('zed', 'PowerShell writes the same provider document as Bash', async t => {
  ...
  t.equal(
    ...
    'claude-opus-4-6[1m],gpt-5.6,plain-chat,effort-only,floor-only,ceiling-only,zero-limits',
