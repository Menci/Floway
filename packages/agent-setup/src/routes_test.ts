...
interface LeaseResponse {
  ...
  configuration: {
    apiKeyId: string;
    claudeCode: { modelDiscovery: boolean; model: string | null; effortLevel: string | null; cleanupPeriodDays: number | null };
...
const FULL_CONFIG_JSON = (apiKeyId: string): string => JSON.stringify({
  apiKeyId,
  claudeCode: { model: null, defaultOpusModel: null, defaultSonnetModel: null, defaultHaikuModel: null, effortLevel: null, cleanupPeriodDays: null, modelDiscovery: true },
...
test('unknown, expired, deleted-user, and deleted-key tokens all return an identical generic 404', async () => {
  const h = harness();
  const now = Date.now();
  const config = '{"apiKeyId":"key_primary","claudeCode":{"model":null,"defaultOpusModel":null,"defaultSonnetModel":null,"defaultHaikuModel":null,"effortLevel":null,"cleanupPeriodDays":null,"modelDiscovery":true},"codex":{"model":null,"reasoningEffort":null}}';
