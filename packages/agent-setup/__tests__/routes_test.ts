...
// happens on the gateway. A listing failure is the operator's to see: an opaque 404 would
...
test('a model listing failure serves a script that reports it', async () => {
  ...
  const lease = await create(h);
  const sh = await h.request(lease.scripts.zed.sh, { method: 'GET' });
  ...
  const ps1 = await h.request(lease.scripts.zed.ps1, { method: 'GET' });
