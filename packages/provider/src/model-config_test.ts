...
test('pricingField rejects malformed entries and duplicate coordinates', () => {
  ...
  assertThrows(() => pricingField({ units: { input: 'tokens_1m' }, entries: [{ rates: {} }] }, 'pricing'), Error, 'at least one rate');
...
describe('modelsField rerank targets', () => {
  ...
  test('rejects targets on other model kinds and kind/endpoint mismatches', () => {
    expect(() => modelsField([{
    ...
    expect(() => modelsField([{
      ...
      endpoints: { chatCompletions: {} },
      rerankTarget: { protocol: 'cohere-v2' },
    }], 'p')).toThrow(/kind === 'rerank' requires endpoints to contain only rerank/);
  });
});
