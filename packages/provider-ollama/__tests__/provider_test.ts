...
test.each(['lite', 'standard'] as const)('Ollama forwards dispatched %s Responses input and projects compact controls on the wire', async transport => {
  const provider = createOllamaProvider(buildRecord({ config: { baseUrl: 'http://127.0.0.1:11434' } }));
  const model = stubProviderModel({ id: 'public-model', providerData: 'upstream-model' });
  ...
  assertEquals(requests.map(request => request.url), ['http://127.0.0.1:11434/v1/responses', 'http://127.0.0.1:11434/v1/responses/compact']);
