...
export const createCodexProvider = (record: UpstreamRecord): Provider => {
  ...
  const instance: ProviderInstance = {
    ...
    callImagesGenerations: async (model, body, signal, opts) => {
      const { account } = await readActiveAccount();
      return await callCodexImagesGenerations({ upstreamId: record.id, account, model, headers: opts.headers, signal, effects, call: opts, body, fallbackPlanType: accountIdentity.planType ?? undefined });
    },
    callImagesEdits: async (model, request, signal, opts) => {
      const { account } = await readActiveAccount();
      return await callCodexImagesEdits({ upstreamId: record.id, account, model, headers: opts.headers, signal, effects, call: opts, request, fallbackPlanType: accountIdentity.planType ?? undefined });
    },
