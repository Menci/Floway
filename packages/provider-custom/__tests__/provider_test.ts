...
test.each(['lite', 'standard'] as const)('Custom forwards dispatched %s Responses input and projects compact controls on the wire', async transport => {
  ...
  assertEquals(requests.map(request => request.url), ['https://custom.example.com/v1/responses', 'https://custom.example.com/v1/responses/compact']);
