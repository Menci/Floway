...
test('OpenAI Responses refusal lifecycle becomes Anthropic Messages refusal metadata without visible text', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
  const events = [
    ...translateOpenAIResponsesStreamEventToAnthropicMessagesEvents({
...
test('OpenAI Responses reasoning stream without readable summary emits a redacted_thinking carrier', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('text-only OpenAI Responses reasoning stream emits a recoverable signature delta', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('OpenAI Responses reasoning stream keeps summary text from deltas when done summary is empty', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('done-only OpenAI Responses reasoning summary stream emits thinking text once', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('done-only OpenAI Responses reasoning summary stream emits every summary part once', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('opaque-only OpenAI Responses reasoning stream releases later text when done', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('OpenAI Responses reasoning stream preserves source order when later reasoning finishes first', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('OpenAI Responses stream keeps later text deferred until earlier tool block is done', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
...
test('terminal OpenAI Responses usage rejects cache splits that exceed input_tokens', () => {
  assertThrows(
    () => terminalUsage({
...
const responseFailedEvent = (error: { code: string; message: string }): Parameters<typeof translateOpenAIResponsesStreamEventToAnthropicMessagesEvents>[0] => ({
  type: 'response.failed',
  response: {
    id: 'resp_ctx',
    object: 'response',
...
test('multiple OpenAI Responses refusal parts compose one Anthropic Messages explanation in output order', () => {
  const state = createOpenAIResponsesToAnthropicMessagesStreamState();
  const events = [
    ...translateOpenAIResponsesStreamEventToAnthropicMessagesEvents({
  ...
  const delta = events.find((event): event is AnthropicMessagesMessageDeltaEvent => event.type === 'message_delta');
