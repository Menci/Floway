...
const drain = async (input: ProtocolFrame<OpenAIChatCompletionsStreamEvent>[]): Promise<void> => {
  await collect(input);
};
...
test('translateToSourceEvents throws on upstream Chat error payloads', async () => {
  await assertRejects(
    ...
    Error,
    'Upstream OpenAI Chat Completions stream error: invalid_request_error: bad request',
  );
});
