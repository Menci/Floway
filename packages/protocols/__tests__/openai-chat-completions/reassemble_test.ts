...
function makeEvents<T = OpenAIChatCompletionsStreamEvent>(chunks: Array<{ event?: string; data: unknown }>): AsyncIterable<T> {
...
test('reassembleOpenAIChatCompletionsEvents reassembles text response', async () => {
  ...
  const result: OpenAIChatCompletionsResult = await reassembleOpenAIChatCompletionsEvents(body);
...
test('reassembleOpenAIChatCompletionsEvents rejects upstream OpenAI Chat Completions error payloads', async () => {
  ...
  await assertRejects(async () => await reassembleOpenAIChatCompletionsEvents(body), Error, 'Upstream OpenAI Chat Completions SSE error: server_error: upstream chat failed');
});

test('reassembleOpenAIChatCompletionsEvents reassembles tool calls', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_2',
...
test('reassembleOpenAIChatCompletionsEvents maintains independent state for every choice index', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_many',
  ...
  const result = await reassembleOpenAIChatCompletionsEvents(body) as OpenAIChatCompletionsResult & {
    choices: Array<OpenAIChatCompletionsResult['choices'][number] & {
      content_filter_results?: unknown;
      message: OpenAIChatCompletionsResult['choices'][number]['message'] & { vendor_trace?: string };
    }>;
  };
...
test('reassembleOpenAIChatCompletionsEvents preserves unknown delta fields by concatenating string streams (reasoning_content)', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_1',
  ...
  const result = await reassembleOpenAIChatCompletionsEvents(body);
  const message = result.choices[0].message as OpenAIChatCompletionsResult['choices'][number]['message'] & { reasoning_content?: string };
  assertEquals(message.content, '391');
  assertEquals(message.reasoning_content, 'I will compute 17 * 23 = 391.');
});

test('reassembleOpenAIChatCompletionsEvents preserves unknown chunk-level fields with last-non-null write wins', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_1',
  ...
  const result = await reassembleOpenAIChatCompletionsEvents(body) as OpenAIChatCompletionsResult & { prompt_filter_results?: unknown };
  assertEquals(result.prompt_filter_results, [{ prompt_index: 0, content_filter_results: { hate: { filtered: false } } }]);
});

test('reassembleOpenAIChatCompletionsEvents preserves unknown choice-level fields (content_filter_results)', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_1',
...
test('reassembleOpenAIChatCompletionsEvents carries system_fingerprint without concatenating its repeated chunks', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_fp',
...
test('reassembleOpenAIChatCompletionsEvents carries service_tier without concatenating its repeated chunks', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_tier',
...
test('reassembleOpenAIChatCompletionsEvents holds first non-empty system_fingerprint when later chunks omit it', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_fp_late',
...
test('reassembleOpenAIChatCompletionsEvents treats a null tool_calls as carrying no call', async () => {
  const body = makeEvents([
    {
      data: {
        id: 'cmpl_null',
...
test('reassembleOpenAIChatCompletionsEvents accumulates refusal deltas separately from content', async () => {
...
test('reassembleOpenAIChatCompletionsEvents preserves an observed empty refusal', async () => {
  const result = await reassembleOpenAIChatCompletionsEvents(makeEvents([{
    data: {
