...
import { mapResponsesTools, type CanonicalResponsesPayload, type ResponsesTool } from '../../src/responses/index.ts';
...
test('maps tools in every Responses request container without mutating the source', () => {
  ...
  const fillDescription = (tool: ResponsesTool): ResponsesTool => tool.type === 'namespace'
    ? { ...tool, description: `Tools in the ${tool.name} namespace.` }
    : tool;

  assertEquals(mapResponsesTools(payload, fillDescription), {
    model: 'gpt-test',
...
test('preserves nullable top-level tools', () => {
  const payload: CanonicalResponsesPayload = { model: 'gpt-test', tools: null, input: [] };
  assertEquals(mapResponsesTools(payload, tool => tool), payload);
});
