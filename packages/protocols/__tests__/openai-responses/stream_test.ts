...
test('parseOpenAIResponsesStream parses OpenAI Responses SSE frames into protocol events', async () => {
...
test('parseOpenAIResponsesStream rejects malformed OpenAI Responses SSE JSON', async () => {
  await assertRejects(
    async () => {
      await collect(parse(
    ...
    Error,
    'Malformed upstream OpenAI Responses SSE JSON for event "response.output_text.delta": not json',
  );
});

test('parseOpenAIResponsesStream expands upstream fast-path (created+in_progress+completed) into the full structured sequence', async () => {
...
test('parseOpenAIResponsesStream passes queued/created/in-progress wrappers through before fast-path expansion', async () => {
  const response = makeResponse('completed');
  const frames = await collect(parse(
...
test('parseOpenAIResponsesStream passes structured upstream events through unchanged when upstream already streams them', async () => {
  const frames = await collect(parse(
    sseFrame(
      JSON.stringify({
...
test('parseOpenAIResponsesStream does not duplicate wrappers when fast-path expansion kicks in', async () => {
...
test('parseOpenAIResponsesStream fills in sequence_number when upstream omits it (monotonic counter)', async () => {
...
test('parseOpenAIResponsesStream advances its counter past upstream-provided sequence numbers (no collisions)', async () => {
...
test('parseOpenAIResponsesStream fast-paths response.failed terminal with error preserved on terminal only', async () => {
  ...
  // the field are normalized by `openaiResponsesStartSnapshot`, which
...
test('parseOpenAIResponsesStream passes raw error terminal through (no .response to expand)', async () => {
  const frames = await collect(parse(
...
test('parseOpenAIResponsesStream fast-paths when ping interleaves the wrappers', async () => {
