...
const getTerminalEventName = (response: OpenAIResponsesResult): 'response.failed' | 'response.incomplete' | 'response.completed' => {
  ...
  case 'in_progress':
  case 'cancelled':
    throw new TypeError(`Cannot expand nonterminal OpenAI Responses status '${response.status}' into terminal events`);
  }
...
const requireItemId = (item: { type: string; id?: string }): string => {
  if (item.id === undefined) throw new Error(`OpenAI Responses ${item.type} output item is missing its id`);
  return item.id;
};
...
const openaiResponsesOutputItemEvents = (item: OpenAIResponsesOutputItem, outputIndex: number): OpenAIResponsesStreamEvent[] => {
  ...
  case 'function_call': return openaiResponsesFunctionCallEvents(item, outputIndex);
  case 'custom_tool_call': return openaiResponsesCustomToolCallEvents(item, outputIndex);
  case 'web_search_call': return openaiResponsesWebSearchCallEvents(item, outputIndex);
  case 'image_generation_call': return openaiResponsesImageGenerationCallEvents(item, outputIndex);
  default: return openaiResponsesGenericOutputItemEvents(item, outputIndex);
  }
};
