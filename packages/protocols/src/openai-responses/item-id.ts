...
// These are only the OpenAI Responses item types Floway itself can create, not a
...
export type GeneratedOpenAIResponsesItemType = keyof typeof generatedItemPrefixes;

export const createRandomOpenAIResponsesItemId = (type: GeneratedOpenAIResponsesItemType): string => {
  if (!Object.hasOwn(generatedItemPrefixes, type)) {
    throw new TypeError(`Unknown generated OpenAI Responses item type: ${type as string}`);
  }
