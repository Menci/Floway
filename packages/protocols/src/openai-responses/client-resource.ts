...
export type ClientOpenAIResponsesTool =
  OpenAIResponsesTool extends infer Tool
    ? Tool extends { type: 'function' }
      ? Tool & Required<Pick<OpenAIResponsesFunctionTool, 'description' | 'parameters' | 'strict'>>
      : Tool
    : never;

// Named for the schema's own `ResponseResource`, hence the singular `Response`
// where the protocol-shaped siblings above carry `OpenAI Responses`.
//
...
export type ClientResponseResource =
  ...
  & {
    ...
    reasoning: ClientOpenAIResponsesReasoning | null;
    usage: ClientOpenAIResponsesUsage | null;
  };
