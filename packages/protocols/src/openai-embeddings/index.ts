...
//
// There is one OpenAI Embeddings protocol, so the canonical form and the wire differ in one
...
export type OpenAIEmbeddingsEncodingFormat = 'float' | 'base64';
...
export type OpenAIEmbeddingsInput = string | readonly string[] | readonly number[] | readonly (readonly number[])[];

export interface CanonicalOpenAIEmbeddingsRequest {
...
export interface ParsedOpenAIEmbeddingsRequest {
  model: string;
...
export interface CanonicalOpenAIEmbedding {
...
export interface CanonicalOpenAIEmbeddingsUsage {
  promptTokens: number;
  totalTokens: number;
}

export interface CanonicalOpenAIEmbeddingsResponse {
  ...
  model: string;
  embeddings: readonly CanonicalOpenAIEmbedding[];
   ...
   *  returned. */
  usage?: CanonicalOpenAIEmbeddingsUsage;
}
...
export interface OpenAIEmbeddingsPayload {
  model: string;
  input: OpenAIEmbeddingsInput;
  encoding_format?: OpenAIEmbeddingsEncodingFormat;
  dimensions?: number;
  user?: string;
}
