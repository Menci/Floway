...
test('OpenAI v1 transports apply api-key auth and the canonical paths', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchChatCompletions(config, { method: 'POST', body: JSON.stringify({ model: 'set-by-provider' }) }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
      await azureFetchResponses(config, { method: 'POST', body: JSON.stringify({ model: 'set-by-provider' }) }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
      await azureFetchResponsesCompact(config, { method: 'POST', body: JSON.stringify({ model: 'set-by-provider' }) }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
      await azureFetchEmbeddings(config, { method: 'POST', body: JSON.stringify({ model: 'set-by-provider' }) }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
    },
  );
...
test('image transports append the Azure preview api-version', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchImagesGenerations(config, { method: 'POST', body: '{}' }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
    },
  );
...
test('endpoint that already includes /openai/v1 routes through unchanged', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchResponses(config, { method: 'POST', body: '{}' }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
    },
  );
...
test('Foundry project endpoints split OpenAI v1 vs Anthropic surfaces', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchResponses(config, { method: 'POST', body: '{}' }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
      await azureFetchMessages(config, { method: 'POST', body: '{}' }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
    },
  );
...
test('Azure Foundry Anthropic surface uses x-api-key + anthropic-version', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchMessages(config, { method: 'POST', body: '{}' }, { extraHeaders: new Headers({ 'anthropic-beta': 'context-1m' }), fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
      await azureFetchMessagesCountTokens(config, { method: 'POST', body: '{}' }, { fetcher: directFetcher, wrapUpstreamCall: <T>(p: Promise<T>): Promise<T> => p });
    },
  );
