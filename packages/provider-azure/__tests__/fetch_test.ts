...
test('Azure Foundry Anthropic surface uses x-api-key + anthropic-version', async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      await azureFetchMessages(config, { method: 'POST', body: '{}' }, { extraHeaders: [['anthropic-beta', 'context-1m']], fetcher: directFetcher, wrapUpstreamCall: identityWrapUpstreamCall });
