...
describe('model badges', () => {
  it('abbreviates the token limits the catalog advertises', () => {
    ...
    expect(modelBadges(model('m', ['a']), indexCatalog([]), null)).toEqual([]);
  });
  ...
  it('collapses a rule its targets disagree on', () => {
    ...
    expect(modelBadges(one, indexCatalog([model('a', ['u']), one]), null)).toContainEqual(
    ...
    expect(modelBadges(many, indexCatalog([model('a', ['u']), model('b', ['u']), many]), null)).toContainEqual(
    ...
    expect(modelBadges(partlyUnset, indexCatalog([model('a', ['u']), model('b', ['u']), partlyUnset]), null)).toContainEqual(
      { key: 'rule:reasoning.effort', kind: 'rule', field: 'reasoning.effort', value: null, varies: true },
    );
    const capped = alias([
      { target_model_id: 'a', rules: { reasoning: { effort: 'high' } } },
    ...
    expect(modelBadges(capped, indexCatalog([model('a', ['u-a']), model('b', ['u-b']), capped]), ['u-a'])).toContainEqual(
      { key: 'rule:reasoning.effort', kind: 'rule', field: 'reasoning.effort', value: 'high', varies: false },
    );
    ...
    expect(modelBadges(disjoint, indexCatalog([model('a', ['u']), model('b', ['u']), disjoint]), null)
