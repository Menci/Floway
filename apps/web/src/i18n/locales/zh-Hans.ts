  ...
  translation: {
    ...
    dashboard: {
      ...
      pages: {
        ...
        oauth2: '配置外部 OAuth2 提供商，允许用户自助登录和注册账号',
      ...
      oauth2: {
        empty: '尚未配置 OAuth2 提供商。添加后即可提供外部登录。',
        callbackUnavailable: '请先设置公开 Base URL，以生成此回调 URL',
        accounts: {
          title: 'OAuth2 账号',
          description: '绑定其他提供商，或移除不再使用的外部身份。',
          adminDescription: '移除此用户绑定的 OAuth2 身份。',
          loading: '正在加载 OAuth2 账号',
          empty: '尚未绑定 OAuth2 账号。',
          bind: '绑定 {{provider}}',
          bound: 'OAuth2 账号已绑定',
          unlink: '解绑',
          unlinked: '已解绑 {{provider}}',
          lastLogin: '请先设置密码或绑定另一个 OAuth2 账号，再解绑最后一种登录方式。',
          unlinkTitle: '解绑 OAuth2 账号',
          unlinkMessage: '解绑 {{provider}} 账号 {{login}}？之后将无法再使用此身份登录。',
        },
        settings: {
          title: '公开控制台来源',
          description: 'Floway 用于 OAuth2 回调和浏览器跳转的外部可访问来源',
          publicBaseUrl: '公开 Base URL',
          hint: '只填写来源，不要包含路径、查询或片段。本地开发可以使用 HTTP。',
          required: '设置此 URL 前，登录页不会显示任何提供商。',
          save: '保存 Base URL',
        },
        status: { enabled: '已启用', disabled: '已禁用' },
        actions: {
          create: '新建 OAuth2 提供商',
          refresh: '刷新 OAuth2 提供商',
          save: '保存提供商',
          delete: '删除提供商',
          editNamed: '编辑 OAuth2 提供商 {{name}}',
          deleteNamed: '删除 OAuth2 提供商 {{name}}',
        },
        table: {
          label: 'OAuth2 登录提供商',
          name: '提供商',
        ...
        dialog: {
          createTitle: '新建 OAuth2 提供商',
          editTitle: '编辑 {{name}}',
        },
        form: {
          enabled: '启用',
          enabledHint: '设置公开 Base URL 后，已启用的提供商会显示在登录页。',
          id: '提供商 ID',
          idHint: '可使用字母、数字、连字符或下划线。ID 会成为回调 URL 的一部分，创建后不能修改。',
          displayName: '显示名称',
          clientId: 'Client ID',
          clientSecret: 'Client Secret',
          clientSecretPreserved: '留空会保留已存储的 Secret。Floway 绝不会把存储值返回给浏览器。',
          authorizationEndpoint: '授权端点',
          tokenEndpoint: 'Token 端点',
          userInfoEndpoint: 'UserInfo 端点',
          accessPolicy: 'UserInfo 准入策略',
          accessPolicyHint: '可选 JSON，将对 UserInfo 响应求值。支持 eq、ne、gt、gte、lt、lte、in、not_in、contains、not_contains、exists 和 not_exists。留空表示允许所有已认证用户。',
          accessPolicyPlaceholder: '{\n  "logic": "or",\n  "conditions": [\n    {\n      "field": "groups",\n      "op": "contains",\n      "value": "organization:team"\n    }\n  ]\n}',
          accessDeniedMessage: '拒绝访问消息',
          accessDeniedMessageHint: '可选模板，在策略拒绝账号时显示。留空使用 Floway 默认消息。变量：{{variables}}。current 是完整的 UserInfo 对象，因此可以用点分路径读取单个 Claim。',
          registrationUpstreams: '自助注册默认上游权限',
          registrationUpstreamsHint: '为之后通过此提供商自助注册的用户设置用户级上游白名单。勾选的上游可用，未勾选的上游不可用；关闭限制表示允许全部上游。现有用户不受影响。',
          scopes: 'Scopes',
          scopesHint: '使用空格分隔 OAuth2 scope，例如：openid profile email groups',
          clientAuthentication: 'Token 端点认证方式',
          userIdClaim: '稳定用户 ID Claim',
          usernameClaim: '建议用户名 Claim',
          claimHint: '可选的 JSON 点分路径。留空时使用 Floway 的标准 Claim 回退顺序。',
          authorizationParams: '附加授权参数',
          authorizationParamsHint: '填写值均为字符串的 JSON 对象。state、redirect_uri 等协议参数由 Floway 管理。',
        },
        clientAuthentication: {
          client_secret_post: '在请求体中发送 Secret',
          client_secret_basic: 'HTTP Basic 认证',
        },
        validation: {
          required: '此字段为必填项。',
          id: '请输入 1-64 位字母、数字、连字符或下划线。',
          maximum: '此值过长。',
          endpoint: '请输入不含凭据的绝对 HTTP 或 HTTPS URL。',
          authorizationParamsJson: '请输入有效的 JSON。',
          authorizationParamsShape: '请输入值均为字符串的 JSON 对象。',
          authorizationParamsReserved: '请移除由 Floway 管理的协议参数。',
          accessPolicyJson: '请输入有效的准入策略 JSON。',
          accessPolicyShape: '请使用 logic（and/or）及受支持的 conditions。exists 和 not_exists 不填写 value；in 和 not_in 需要数组；大小比较需要数字或字符串。',
        },
        toast: {
          settings: { pending: '正在保存 OAuth2 Base URL', success: '已保存 OAuth2 Base URL' },
          create: { pending: '正在创建 OAuth2 提供商 {{name}}', success: '已创建 OAuth2 提供商 {{name}}' },
          edit: { pending: '正在保存 OAuth2 提供商 {{name}}', success: '已保存 OAuth2 提供商 {{name}}' },
          delete: { pending: '正在删除 OAuth2 提供商 {{name}}', success: '已删除 OAuth2 提供商 {{name}}' },
        },
        delete: {
          title: '删除 OAuth2 提供商',
          message: '删除 {{name}}？现有账号绑定会保留，但在以相同提供商 ID 重新创建配置前，用户无法再通过它登录。',
        },
      },
      upstreamAccess: {
        title: '限制可用上游',
      ...
      backupRestore: {
        ...
        import: {
          ...
          imported: {
            ...
            oauth2Providers_other: '{{count, number}} 个 OAuth2 提供商',
