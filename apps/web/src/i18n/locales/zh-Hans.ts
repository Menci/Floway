    ...
    dashboard: {
      ...
      upstreamEditor: {
        ...
        codex: {
          ...
          resetCards: {
            title: '重置卡',
            available: '{{count, number}} 张可用',
            refresh: '刷新重置卡',
            defaultTitle: 'Codex 限额重置',
            granted: '发放于 {{time}}',
            expires: '到期于 {{time}}',
            noExpiry: '上游未提供到期时间',
            use: '使用重置卡',
            ...
            confirmMessage: '现在使用“{{title}}”？这会重置符合条件的 Codex 限额窗口，且无法撤销。',
            refreshWarning: '重置卡已处理，但 Floway 未能刷新所有详情。请刷新重置卡以查看当前状态。',
            loadError: '无法加载重置卡，请再次刷新。',
            redeemError: '无法确认重置结果。重试将安全地检查同一次兑换。',
            result: {
              reset: 'Codex 限额窗口已重置。',
              alreadyRedeemed: '此请求已兑换过，没有再次消耗重置卡。',
              nothingToReset: '当前没有符合条件的限额窗口可供重置。',
              noCredit: '这张重置卡已不可用。',
              unknown: 'OpenAI 返回了 Floway 尚不认识的重置结果：{{code}}',
            },
          },
          import: {
            tabJson: '粘贴 JSON',
      ...
      requests: {
        ...
        findEvents: '搜索事件',
        eventOptions: '事件选项',
        collapseEvents: '折叠全部事件',
        expandEvents: '展开全部事件',
        noEventMatches: '没有匹配的事件',
        events: '事件（{{count, number}}）',
        headers: '请求头 / 响应头（{{count}}）',
        raw: '原始内容',
        base64: '原始字节（Base64）',
        partialCapture: '采集在 EOF 前结束。这里保留了目前收到的字节。',
        ...
        exportRecord: '导出记录',
        search: '搜索 ID、模型、路径、上游、状态或错误',
        failuresOnly: '仅看失败',
        applyFilters: '搜索历史记录',
        selectExport: '选择导出记录',
        exportCombined: '合并为 JSON',
        exportSeparate: '分别导出（.tar）',
        selectLoaded: '选择已加载记录',
        clearSelection: '取消选择',
        exportSelection: '导出已选 {{count}} 条',
        exportFailed: '无法导出所选记录。',
        exporting: '正在准备导出…',
      },
      charts: {
        series: {
      ...
      modelAliases: {
        ...
        target: { heading: '模型', description: '使用“首个可用”时将按顺序尝试目标。可选择建议或输入任意模型 ID。', label: '目标 {{number, number}}', modelId: '目标模型 ID', placeholder: '目标模型 ID', toggle: '展开目标规则', reorder: '拖动排序目标', remove: '移除目标', count_other: '{{count, number}} 个目标' },
