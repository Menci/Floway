    ...
    dashboard: {
      ...
      apiKeys: {
        ...
        configuration: {
          ...
          vscodeConfigHint: '将此 group 添加到 <path>chatLanguageModels.json</path>，它位于 VS Code profile 目录中 <path>settings.json</path> 旁边。如果已有同名 group，请替换它 —— 同名的两个 group，VS Code 只会保留第一个。',
          vscodeAuthHint: '密钥写在每个模型的请求头中，因为 group 自身的密钥字段会被当作指向 secret storage 的引用来解析。密钥以明文保存在该文件里，请将文件权限限制为仅自己可读——安装脚本会自动这样做。',
        },
        agentSetup: { agent: 'Agent', accessMethod: '接入方式', setupTab: '自动配置脚本', snippetsTab: '配置文件片段', platform: '操作系统', commandPending: '正在准备安装命令…', modelSelection: '模型选择', miscSettings: '杂项设置', selectKey: '请先在上方选择 API 密钥。', noKey: '请先创建 API 密钥。', expired: '此安装链接已过期，请重试生成新链接。', timedOut: '网关未在规定时间内响应。', retry: '重试', expires: '页面可见时链接会自动续期，离开后即过期。', defaultModel: '默认模型', fableModel: 'Fable 模型', opusModel: 'Opus 模型', sonnetModel: 'Sonnet 模型', haikuModel: 'Haiku 模型', reasoningEffort: '思考强度', modelDefault: '默认', noModelMatches: '没有匹配的模型', modelDiscovery: 'Gateway 模型发现', modelDiscoveryHint: '允许 Claude Code 从此 Floway gateway 发现可用模型。', cleanupRetention: '清理保留期', cleanupRetentionHint: '设置 Claude Code 本地会话数据的清理保留期。', cleanupDays: '{{count, number}} 天', optOutAiAttribution: '停用 Claude Code AI 归属标记', optOutAiAttributionHint: '移除提交和 Pull Request 中的 Claude Code 归属信息（"Co-Authored-By"），并隐藏会话链接。', disableAutoMemory: '停用自动记忆', disableAutoMemoryHint: '禁止 Claude Code 读写其自动记忆目录。', disableAgentView: '停用 Agent 视图', disableAgentViewHint: '关闭后台 agent 与 agent 视图，包括 `claude agents`、`--bg` 和 `/background`。', providerName: 'Provider 名称', zedNoChatModels: '此网关提供的对话模型暂时都无法配置到 Zed。请先添加提供对话模型的上游。', vscodeNoChatModels: '此网关提供的对话模型暂时都无法配置到 VS Code。请先添加提供对话模型的上游。', zedProviderNameHint: '在 Zed 的模型选择器中标识此网关。修改后需重新运行安装；旧条目在你手动移除前会保留。', vscodeProviderNameHint: '在 VS Code 的 provider 列表中标识此网关的 group。修改后需重新运行安装；旧 group 在你手动移除前会保留。', providerNameInvalid: '请输入名称，首尾不能有空格，且不能包含控制字符。', zedModelSnapshot: 'Zed 无法自行发现模型，安装时会写入当前模型列表。模型列表变化后请重新运行安装。', apiType: 'API', apiTypeHint: 'VS Code 访问哪条 API 路径。Floway 对每个模型都提供全部三种。在 VS Code 中选择的推理强度会随 Chat Completions 和 Responses 发送，Messages 路径则会丢弃它。', vscodeModelSnapshot: 'VS Code 无法自行发现模型，安装时会将当前模型列表写入所有已安装的版本和 profile。模型列表变化后请重新运行安装。' },
        rotate: {
          title: '轮换 API 密钥',
