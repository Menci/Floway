...
// under the pointer, and the focus rect is XAML's universal one, drawn inside
// the name's own box because the cell clips whatever leaves it.
...
export const useRowTitleClass = (): string => mergeClasses(useStyles().root, 'text-fui-fg1 no-underline hover:underline');
...
export function RowTitleButton({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  const paint = useRowTitleClass();
  return <button className={`${paint} block bg-transparent border-0 cursor-pointer min-w-0 max-w-full truncate p-0 text-fui-base300 text-left`} onClick={onClick} type="button">
    {children}
  </button>;
}
