...
test('ModelsPanel validates every manual row before it is selected', async () => {
  const valid = model('valid', { units: { input: 'tokens_1m' }, entries: [{ rates: { input: 1 } }] });
  const invalid = model('invalid', { units: {}, entries: [] });
  ...
  await wrapper.setProps({ modelValue: [valid, model('fixed', { units: { input: 'tokens_1m' }, entries: [{ rates: { input: 2 } }] })] });
  await nextTick();
